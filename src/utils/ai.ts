...
const actionInstructionMap: Record<AiAction, string> = {
  polish:
    '润色表达，让语言更精炼、更有冲击力，同时保持事实不变。必须重写句式，不得原样返回。每句话尽量包含「做了什么」+「怎么做的」+「取得了什么结果」。',
  expand:
    '基于原文合理扩写，补充具体做法、工作流程、技术细节或业务价值。必须比原文更具体、更完整，不得原样返回。用 STAR 或 PAR 结构（背景-行动-结果）组织内容；严禁编造未提及的公司、项目和数字。',
  shorten:
    '压缩为最精炼的简历 bullet points，保留核心成果、关键技术和影响力。每点不超过 25 字，适合排版紧凑的简历。不得原样返回。',
  professional:
    '改写为资深 HR 偏好的职业化表达：强动词开头、结果导向、去口语化。必须替换弱表达，不得原样返回；如果原文没有数据，不要编造数字。避免"负责日常""参与一些"等弱表达。',
  diagnose:
...
export function buildResumeAiPrompt({ action, section, text, targetRole, jdText }: BuildResumeAiPromptInput): string {
  ...
  parts.push(
    ...
    '2. 不得原样返回：即使原文很短，也必须改变句式、增加信息密度或转换为更适合简历的表达。',
    '3. 严禁编造学校、公司、岗位、证书、具体项目名、具体数字等事实。如果原文没有量化数据，不要编造数字，可用「提升交付质量」「优化用户体验」「保障功能落地」等非数字成果表达。',
    '4. 如果信息不足，只基于原文可确认的信息进行职业化改写；可以补充通用工作动作（如需求理解、方案落地、协作推进、质量优化），不要添加无法确认的技术栈/金额/比例/人数。',
    '5. 中文表达自然、专业，不要出现 AI 腐蚀式语言。',
    '6. 若原文为多条目，保持分点列表形式（每条以换行分隔）。',
    '7. 尽量针对目标岗位突出相关关键词和能力。',
    '',
    '---',
    `请优化以下内容：`,
    text.trim(),
